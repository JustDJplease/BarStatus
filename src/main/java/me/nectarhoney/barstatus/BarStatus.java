...
public class BarStatus extends JavaPlugin {
    ...
    public void onEnable() {
        saveDefaultConfig();
        importConfig();
        getCommand("togglerankbar").setExecutor(new BarCommand(this));
    ...
    private void importConfig() {
        disabledPlayers = getConfig().getStringList("disabled");
    }

    boolean hasBarDisabled(UUID uuid) {
        return disabledPlayers.contains(uuid.toString());
    }

    void toggleBar(UUID uuid) {
        if (hasBarDisabled(uuid)) {
            disabledPlayers.remove(uuid.toString());
        } else {
            disabledPlayers.add(uuid.toString());
        }
        getConfig().set("disabled", disabledPlayers);
        saveConfig();
    }
}
